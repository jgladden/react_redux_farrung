import section, { initialState } from '../../src/reducers/section';
...
describe('section reducers', () => {
  ...
  test('set section secondary returns expected state', () => {
    const action = { type: SET_SECTION, payload: { primary: 'portfolio', secondary: 'online' } };
    expect(section(undefined, action)).toMatchSnapshot();
  });
  test('set section tertiary returns expected state', () => {
    const action = { type: SET_SECTION, payload: { primary: 'portfolio', secondary: 'online', tertiary: '22' } };
    expect(section(undefined, action)).toMatchSnapshot();
  });
});
